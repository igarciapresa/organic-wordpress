...
function azc_tc_load_plugin_textdomain(){
	$loaded = load_plugin_textdomain( 'azc-tc', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	//if ($loaded){ echo 'true'; }else{ echo 'false'; }
}
add_action('plugins_loaded', 'azc_tc_load_plugin_textdomain');

function azc_tc_load_css(){
	wp_enqueue_style( 'azc-tc', plugins_url( 'style.css', __FILE__ ) );
}
...
function azc_tc_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=azc-tc">Settings</a>';
...
function azc_tc_settings() {
	if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.', 'azc-tc'));
    }
	...
	<div id="azc-tc-general" class="wrap">
		<fieldset>
			<h2>azurecurve Tag Cloud <?php _e('Settings', 'azc-tc'); ?></h2>
			...
			<form method="post" action="admin-post.php">
				...
				<tr><th scope="row"><label for="include_exclude"><?php _e('Include/Exclude Tags?', 'azc-tc'); ?></label></th><td>
					...
					<p class="description"><?php _e('Flag whether marked tags should be included or excluded from the tag cloud', 'azc-tc'); ?></p>
				</td></tr>
				...
				<tr><th scope="row">Tags to Include/Exclude</th><td>
					...
					<p class="description"><?php _e('Mark the tags you want to include/exclude from the tag cloud', 'azc-tc'); ?></p>
				</td></tr>
				
				<?php if (function_exists('is_multisite') && is_multisite()) { ?>
					<tr><th scope="row">Use Network Settings</th><td>
						<fieldset><legend class="screen-reader-text"><span><?php _e('Use Network Settings', 'azc-tc'); ?></span></legend>
						<label for="use_network_settings"><input name="use_network_settings" type="checkbox" id="use_network_settings" value="1" <?php checked( '1', $options['use_network_settings'] ); ?> /><?php _e('Use Network Settings? The settings below will be ignored', 'azc-tc'); ?></label>
						</fieldset>
					</td></tr>
				<?php } ?>
				<tr><th scope="row"><label for="smallest"><?php _e('Smallest Size', 'azc-tc'); ?></label></th><td>
					<input type="text" name="smallest" value="<?php echo esc_html( stripslashes($options['smallest']) ); ?>" class="small-text" />
					<p class="description"><?php _e('The text size of the tag with the lowest count value', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="largest"><?php _e('Largest Size', 'azc-tc'); ?></label></th><td>
					<input type="text" name="largest" value="<?php echo esc_html( stripslashes($options['largest']) ); ?>" class="small-text" />
					<p class="description"><?php _e('The text size of the tag with the highest count value', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="unit"><?php _e('Unit', 'azc-tc'); ?></label></th><td>
					<select name="unit">
					...
					<p class="description"><?php _e('Unit of measure as pertains to the smallest and largest values', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="number"><?php _e('Number Of Tags', 'azc-tc'); ?></label></th><td>
					<input type="text" name="number" value="<?php echo esc_html( stripslashes($options['number']) ); ?>" class="small-text" />
					<p class="description"><?php _e('The number of actual tags to display in the cloud', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="format"><?php _e('Format', 'azc-tc'); ?></label></th><td>
					<select name="format">
					...
					<p class="description"><?php _e('Format of the cloud display', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="orderby"><?php _e('Order By', 'azc-tc'); ?></label></th><td>
					<select name="orderby">
					...
					<p class="description"><?php _e('Order of the tags', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="order"><?php _e('Order', 'azc-tc'); ?></label></th><td>
					<select name="order">
					...
					<p class="description"><?php _e('Sort order', 'azc-tc'); ?></p>
				</td></tr>
				<tr><th scope="row"><label for="widget"><?php _e('Plugin Widget', 'azc-tc'); ?></label></th><td>
					<p class="description"><?php _e(sprintf('Once you\'ve saved the settings, make sure you place the %s widget in a widget area', 'azurecurve Tag Cloud'), 'azc-tc'); ?></p>
				</td></tr>
				</table>
...
function process_azc_tc_options() {
	...
	if ( !current_user_can( 'manage_options' ) ){
		wp_die( __('You do not have permissions to perform this action', 'azc-tc') );
	}
...
function azc_tc_network_settings_page(){
	if(!current_user_can('manage_network_options')) wp_die(__('You do not have permissions to perform this action', 'azc-tc'));
	$options = get_site_option('azc_tc_options');

	?>
	<div id="azc-tc-general" class="wrap">
		<fieldset>
			<h2><?php _e('azurecurve Tag Cloud Settings', 'azc-tc'); ?></h2>
			<form method="post" action="edit.php?action=update_azc_tc_network_options">
				<input type="hidden" name="action" value="save_azc_tc_options" />
				<input name="page_options" type="hidden" value="smallest, largest, number" />
...
class azurecurve_tag_cloud extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'azurecurve_tag_cloud',
							 'azurecurve Tag Cloud',
							 array( 'description' =>
									__('A customizable cloud of your most used tags.', 'azc-tc') ) );
	}
	...
	public function enqueue() {
		// Enqueue Styles
		wp_enqueue_style( 'azc-tc', plugins_url( 'style.css', __FILE__ ), '', '1.0.0' );
	}
...
// azurecurve menu
function azc_create_tc_plugin_menu() {
...
add_action("admin_menu", "azc_create_tc_plugin_menu");
